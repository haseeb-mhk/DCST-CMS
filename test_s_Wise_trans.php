...
//	selecting the subjects offered in the given semester
	$assign_sub = mysqli_query($con,"SELECT * FROM assign_subject WHERE Semester_Id ='$sem_Id'");
...
//		print_r(count($subject_id_s));
 ...
 WHERE Subject_Id ='$a_s_id' AND Roll_No='$roll_n'");
	...
	<?php 
		 ...
		 <td><?php echo($t_ob_marks)?></td>
		 <td><?php echo round($t_ob_marks*100/($a*100), 1);  ?>%</td>
		 <td> <?php 
		$gpa = $sum_gp /$sum_cr;
		echo( round($gpa,2));
		?></td>
